...
var Validate = function() {
	var handleProjectCategory = function() {
		$("#frmProjectCategory").validate({
			errorElement: 'span',
			...
			invalidHandler: function(event, validator) { //display error alert on form submit 
				...
				$('.alert-danger', $('#frmProjectCategory')).show();
			},
		...
		$('#frmProjectCategory input').keypress(function(e) {
			if (e.which == 13) {
				if ($('#frmProjectCategory').validate().form()) {
					$('#frmProjectCategory').submit(); //form validation success, call ajax form submit
				}
				return false;
			}
		});
...
jQuery(document).ready(function() {   	 
	...
	$("#parent_category_id").addClass('bs-select').addClass('select2');
});
