	...
	var handleServiceCategory = function() {
		$("#frmCareersCategory").validate({
			...
			invalidHandler: function(event, validator) { //display error alert on form submit
				...
				$('.alert-danger', $('#frmCareersCategory')).show();
			},
		...
		$('#frmCareersCategory input').keypress(function(e) {
			if (e.which == 13) {
				if ($('#frmCareersCategory').validate().form()) {
					$('#frmCareersCategory').submit(); //form validation success, call ajax form submit
				}
				return false;
			}
		});
