		...
		var handleRecords = function() {
				...
				grid.init({
						...
						dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
								...
								"ajax": {
										"url": window.site_url + "/powerpanel/blog-category/get_list", // ajax source
								},
				...
				$(document).on('keyup', '#searchfilter', function(e) {
						...
						if (action.length >= 2) {
								//$.cookie('BlogCategorySearch',action);				
								grid.setAjaxParam("customActionType", "group_action");
								grid.setAjaxParam("searchValue", action);
								grid.setAjaxParam("id", grid.getSelectedRows());
								grid.getDataTable().ajax.reload();
						} else if (action.length < 1) {
								//$.removeCookie('BlogCategorySearch');
								grid.setAjaxParam("customActionType", "group_action");
								grid.setAjaxParam("searchValue", action);
								grid.setAjaxParam("id", grid.getSelectedRows());
								grid.getDataTable().ajax.reload();
...
function reorder(curOrder, excOrder) {
		var ajaxurl = site_url + '/powerpanel/blog-category/reorder';
		$.ajax({
				url: ajaxurl,
				data: {
						order: curOrder,
