 ...
 */
 var Validate = function() {
		var handleNewsCategory = function() {
				 $("#frmNewsCategory").validate({
					...
					invalidHandler: function(event, validator) { //display error alert on form submit
						    ...
						    if (errors) {
								...
								$('.alert-danger', $('#frmNewsCategory')).show();
						},
				...
				$('#frmNewsCategory input').keypress(function(e) {
						if (e.which == 13) {
								if ($('#frmNewsCategory').validate().form()) {
										$('#frmNewsCategory').submit(); //form validation success, call ajax form submit
								}
								return false;
						}
				});
