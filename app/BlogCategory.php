...
class BlogCategory extends Model {  
  ...
  {
      $response = false;
      $response = Cache::tags(['BlogCategory'])->get('getBlogCatRecordIdByAliasID_'.$aliasID);
      if(empty($response)){
      	$response = Self::Select('id')->deleted()->publish()->checkAliasId($aliasID)->first();
      	Cache::tags(['BlogCategory'])->forever('getBlogCatRecordIdByAliasID_'.$aliasID, $response);
      }
      return $response;
  }
	...
	public static function getFrontList(){
		...
		$response = Cache::tags(['BlogCategory'])->get('getBlogCatFrontList');		
		if(empty($response)){
			...
			->get()
			->pluck('varTitle','id');
			Cache::tags(['BlogCategory'])->forever('getBlogCatFrontList', $response);
			$response = Cache::tags(['BlogCategory'])->get('getBlogCatFrontList');
		}
		return $response;
	}
	...
	*/
	public function blog(){
		$response = false;
		$response = $this->hasOne('App\Blog','id','intCategoryId');
		return $response;
	}
	...
	/**
		...
		*/
		public function blogCategory(){
			$response = false;
			$response = $this->hasOne('App\BlogCategory','id','intParentCategoryId');
			return $response;
		}
		...
		*/
		public function parentCategory() {
			$response = false;
			$response = $this->belongsTo('App\BlogCategory', 'intParentCategoryId', 'id');
			return $response;
		}
	...
	{
		$response = false;
		$categoryFields=['id', 'varTitle' , 'intAliasId', 'intParentCategoryId','intDisplayOrder','txtShortDescription','txtDescription','varMetaTitle','varMetaKeyword','varMetaDescription','chrPublish'];
		$aliasFields = ['id','varAlias'];
	...
	{
		$response = false;
		$blogCategoryPageFields=['id', 'varTitle' , 'intAliasId', 'intParentCategoryId'];
		$aliasFields = ['id','varAlias'];
		$response =  Self::getPowerPanelRecords($blogCategoryPageFields,$aliasFields)
		->deleted()
		->publish()
		->get()
		->toArray();
