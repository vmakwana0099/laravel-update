...
/**
		...
		{
				$response      = false;
				$moduleFields = ['id','varTitle','varEmail','varPhoneNo','txtComments','chrDelete','varIpAddress','created_at','updated_at'];       
	...
	{     
		...
		if ($eventFields != false) {
				$data['event'] = function ($query) use ($eventFields) {$query->select($eventFields);};
		}
		if(count($data)>0){
	...
	public static function getRecordList($filterArr=false){
		$response = false;
		$moduleFields=[ 'id','fkIntEventId','varTitle','varEmail','varPhoneNo','txtComments','chrInterest','intNoOfPeople','dtCommingDateTime','varIpAddress','created_at','chrPublish'];
		$eventFields = ['id','varTitle'];
		$response = Self::getPowerPanelRecords($moduleFields,$eventFields)
		->deleted()
		->filter($filterArr)
		->get(); 
		return $response;
	...
	public static function getListForExport($selectedIds=false){
		$response = false;
		$moduleFields=[ 'varTitle','fkIntEventId','varEmail','varPhoneNo','txtComments','chrInterest','intNoOfPeople','dtCommingDateTime','varIpAddress','created_at'];
		$eventFields = ['id','varTitle'];
		$query = Self::getPowerPanelRecords($moduleFields,$eventFields)->deleted();
	...
	/**
		...
		*/
		...
		{
				$response = false;
				$response = $this->belongsTo('App\Events', 'fkIntEventId', 'id');
				return $response;
		}
