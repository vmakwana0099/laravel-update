...
class ContactleadController extends PowerpanelController {
    ...
    public function index() {
        ...
        $Category = $iTotalRecords > 0 ? ContactLead::getCategoriesAllData() : null;
        $this->breadcrumb['title'] = trans('template.contactleadModule.manageContactLeads');
        return view('powerpanel.contact_lead.list', ['iTotalRecords' => $iTotalRecords, 'breadcrumb' => $this->breadcrumb, 'CatData' => $CatData, 'Category' => $Category]);
    }
    ...
    public function ExportRecord() {
        ...
        if (Input::get('export_type') == 'selected_records') {
            ...
            $arrResults = ContactLead::getListForExport();
        }

        if (count($arrResults) > 0) {
            foreach ($arrResults as $key => $value) {

                $selCategory = ContactLead::getCategoriesData($value->fkIntServiceId);
...
//                    $phoneNo = $value->varPhoneNo;
    ...
    public function createContactLeadExcel($data) {
        ...
        Excel::create(Config::get('Constant.SITE_NAME') . '-' . trans("template.contactleadModule.contactUsLeads") . '-' . date("dmy-h:i"), function($excel) use($data) {
            $excel->sheet(date('M-d-Y'), function($sheet) use($data) {
                $sheet->setAutoSize(true);
                $sheet->fromArray($data);
                $sheet->row(1, array(
                    trans('template.common.name'),
                    trans('template.contactleadModule.type'),
                    trans('template.common.email'),
                    trans('template.contactleadModule.phone'),
                    trans('template.contactleadModule.message'),
                    trans('template.contactleadModule.ipAddress'),
                    trans('template.contactleadModule.receivedDateTime')
                ));

                $sheet->prependRow(array(
                    Config::get('Constant.SITE_NAME') . ' ' . trans("template.contactleadModule.contactUsLeads")
                ));
    ...
    public function tableData($value) {
        $details = '';
        $phoneNo = '';
        // echo '<pre>selCategory: '; print_r($value['fkIntServiceId']);exit;
        $selCategory = ContactLead::getCategoriesData($value->fkIntServiceId);
