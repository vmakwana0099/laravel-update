...
class TestimonialsController extends PowerpanelController {
    ...
    public function index() {
        ...
        $this->breadcrumb['title'] = trans('template.testimonialsModule.manageTestimonials');
        $breadcrumb = $this->breadcrumb;
        return view('powerpanel.testimonials.index', compact('iTotalRecords', 'breadcrumb', 'CatData'));
    }
    ...
    public function edit($id = false) {
        ...
        if (!is_numeric($id)) {
            ...
            $total = $total + 1;
            $this->breadcrumb['title'] = trans('template.testimonialsModule.manageTestimonials');
            $this->breadcrumb['module'] = trans('template.testimonialsModule.manageTestimonials');
            $this->breadcrumb['url'] = 'powerpanel/testimonials';
            $this->breadcrumb['inner_title'] = trans('template.testimonialsModule.addTestimonials');
            $breadcrumb = $this->breadcrumb;
            $data = compact('total', 'breadcrumb', 'imageManager','videoManager');
        } else {
            $testimonials = Testimonials::getRecordById($id);
            // echo '<pre>';print_r($testimonials);exit;
            if (count((array)($testimonials)) == 0) {
                return redirect()->route('powerpanel.testimonials.add');
            }
            $metaInfo = array('varMetaTitle' => $testimonials->varMetaTitle, 'varMetaKeyword' => $testimonials->varMetaKeyword, 'varMetaDescription' => $testimonials->varMetaDescription);
            $this->breadcrumb['title'] = trans('template.testimonialsModule.editTestimonials') . ' - ' . $testimonials->varTitle;
            $this->breadcrumb['module'] = trans('template.testimonialsModule.manageTestimonials');
            $this->breadcrumb['url'] = 'powerpanel/testimonials';
            $this->breadcrumb['inner_title'] = trans('template.testimonialsModule.editTestimonials') . ' - ' . $testimonials->varTitle;
            $breadcrumb = $this->breadcrumb;
            $data = compact('testimonials',  'metaInfo', 'breadcrumb', 'imageManager','videoManager');
        }

        return view('powerpanel.testimonials.actions', $data);
    }
    ...
    public function handlePost(Request $request) {
        ...
        $messsages = array(
            'order.greater_than_zero' => trans('template.testimonialsModule.displayGreaterThan'),
        ...
        if ($validator->passes()) {
            ...
            $testimonialsArr = [];
            $testimonialsArr['varTitle'] = trim($data['title']);
            $testimonialsArr['txtClientUrl'] = trim($data['clientUrl']);
            $testimonialsArr['varRatings'] = trim($data['ratings']);
            $testimonialsArr['fkIntImgId'] = !empty($data['img_id']) ? $data['img_id'] : null;
            $testimonialsArr['txtShortDescription'] = trim($data['short_description']);
            $testimonialsArr['txtDescription'] = trim($data['description']);
            // $testimonialsArr['txtHomePageDesc'] = $data['homepage_description'];
            $testimonialsArr['fkIntVideoId'] = !empty($data['video_id'])?$data['video_id']:null;
            // $testimonialsArr['varVideoLink'] = $data['video_link'];
            $testimonialsArr['varMetaTitle'] = trim($data['varMetaTitle']);
            $testimonialsArr['varMetaKeyword'] = trim($data['varMetaKeyword']);
            $testimonialsArr['varMetaDescription'] = trim($data['varMetaDescription']);
            $testimonialsArr['chrPublish'] = isset($data['chrMenuDisplay']) ? $data['chrMenuDisplay'] : 'Y';
            
            $testimonialsArr['varIpAddress'] = $request->ip();
            $testimonialsArr['varRefURL'] = $data['varRefURL'];

            $testimonialsArr['chrDisplayonhomepage'] = isset($data['displayonhome']) ? $data['displayonhome'] : "N";
            ...
            if (is_numeric($id)) { #Edit post Handler=======
                ...
                $testimonials = Testimonials::getRecordForLogById($id);
                $whereConditions = ['id' => $testimonials->id];
                // echo '<pre>';print_r($whereConditions);exit;
                $update = CommonModel::updateRecords($whereConditions, $testimonialsArr);
                if ($update) {

                    if (!empty($id)) {
                        self::swap_order_edit($data['display_order'], $testimonials->id);

                        $logArr = MyLibrary::logData($testimonials->id);
                        if (Auth::user()->can('log-advanced')) {
                            $newTestimonialsObj = Testimonials::getRecordForLogById($testimonials->id);
                            $oldRec = $this->recordHistory($testimonials);
                            $newRec = $this->recordHistory($newTestimonialsObj);
                        ...
                        if (Auth::user()->can('recent-updates-list')) {
                            if (!isset($newTestimonialsObj)) {
                                $newTestimonialsObj = Testimonials::getRecordForLogById($testimonials->id);
                            }
                            $notificationArr = MyLibrary::notificationData($testimonials->id, $newTestimonialsObj);
                            RecentUpdates::setNotification($notificationArr);
                        }
                    }
                    self::flushCache();
                    $actionMessage = trans('template.testimonialsModule.updateMessage');
                }
            } else { 


                #Add post Handler=======
                $testimonialsArr['intAliasId'] = MyLibrary::insertAlias($data['alias']);
                $testimonialsArr['intDisplayOrder'] = self::swap_order_add($data['display_order']);
                $testimonialsID = CommonModel::addRecord($testimonialsArr);
                if (!empty($testimonialsID)) {
                   
                    $id = $testimonialsID;
                    $newTestimonialsObj = Testimonials::getRecordForLogById($id);
                    $logArr = MyLibrary::logData($id);
                    $logArr['varTitle'] = $newTestimonialsObj->varTitle;

                    Log::recordLog($logArr);
                    if (Auth::user()->can('recent-updates-list')) {
                        $notificationArr = MyLibrary::notificationData($id, $newTestimonialsObj);
                        RecentUpdates::setNotification($notificationArr);
                    }

                     self::flushCache();   
                    $actionMessage = trans('template.testimonialsModule.addMessage');
...
//            AddDocumentModelRel::sync(explode(',', $data['doc_id']), $id);
            if (!empty($data['saveandexit']) && $data['saveandexit'] == 'saveandexit') {
                return redirect()->route('powerpanel.testimonials.index')->with('message', $actionMessage);
            } else {
                return redirect()->route('powerpanel.testimonials.edit', $id)->with('message', $actionMessage);
            }
        } else {
    ...
    public function get_list() {
        ...
        $arrResults = Testimonials::getRecordList($filterArr);
        $iTotalRecords = CommonModel::getRecordCount($filterArr, true);

        $end = $filterArr['iDisplayStart'] + $filterArr['iDisplayLength'];
        $end = $end > $iTotalRecords ? $iTotalRecords : $end;

        /*echo '<pre>vinod';print_r($arrResults);exit;*/
        if (!empty($arrResults)) {
            foreach ($arrResults as $key => $value) {
                $records["data"][] = $this->tableData($value);
            }
        }
        /*echo '<pre>212121 ';print_r($records["data"]);exit;*/
    ...
    public function tableData($value = false) {
        $moduleData = Modules::getModule('testimonials'); $moduleId = $moduleData['id'];
        $webHits = Pagehit::where('isWeb', 'Y')->where('fkIntAliasId', $value->id)->where('intFKModuleCode',$moduleId)->count();
        $mobileHits = Pagehit::where('isWeb', 'N')->where('fkIntAliasId', $value->id)->where('intFKModuleCode',$moduleId)->count();
        ...
        if (Auth::user()->can('testimonials-edit')) {
            $details .= '<a class="without_bg_icon" title="' . trans("template.common.edit") . '" href="' . route('powerpanel.testimonials.edit', array('alias' => $value->id)) . '"><i class="fa fa-pencil"></i></a>';
        }
        if (Auth::user()->can('testimonials-delete')) {
            $details .= '&nbsp;<a class="without_bg_icon delete" title="' . trans("template.common.delete") . '" data-controller="testimonials" data-alias = "' . $value->id . '"><i class="fa fa-times"></i></a>';
        ...
        if (Auth::user()->can('testimonials-publish')) {
           
                if ($value->chrPublish == 'Y') {
                    $publish_action .= '<input data-off-text="No" data-on-text="Yes" class="make-switch publish" class="make-switch publish" data-off-color="info" data-on-color="primary" type="checkbox" data-controller="powerpanel/testimonials" title="' . trans("template.common.publishedRecord") . '" data-value="Unpublish" data-alias="' . $value->id . '">';
                } else {
                    $publish_action .= '<input checked="" data-off-text="No" data-on-text="Yes" class="make-switch publish" class="make-switch publish" data-off-color="info" data-on-color="primary" type="checkbox" data-controller="powerpanel/testimonials" title="' . trans("template.common.unpublishedRecord") . '" data-value="Publish" data-alias="' . $value->id . '">';
                }
        ...
        $records = array(
             $checkbox,
            '<a class="without_bg_icon" title="' . trans("template.common.edit") . '" href="' . route('powerpanel.testimonials.edit', array('alias' => $value->id)) . '">' . $value->varTitle . '</a>',            
    ...
    public function recordHistory($data = false) {
        ...
        $returnHtml .= '<td>' . '<img height="50" width="50" src="' . resize_image::resize($data->fkIntImgId) . '" />' . '</td>
                                                ...
                                                // echo '<pre>';print_r($returnHtml);exit;
        return $returnHtml;
    }
