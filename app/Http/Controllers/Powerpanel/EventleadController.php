...
class EventleadController extends PowerpanelController {
	...
	*/
	...
	public function index() 
	{
		...
		$Events = 
		$this->breadcrumb['title']= trans('template.eventleadModule.manageEventLeads');
		return view('powerpanel.event_lead.list',['iTotalRecords'=>$iTotalRecords,'breadcrumb'=>$this->breadcrumb]);
	}
	...
	/**
 * This method handels export process of event us leads
 * @return  xls file
 * @since   2016-10-18
 * @author  NetQuick
 */
	public function ExportRecord() 
	{
		if (Input::get('export_type')=='selected_records') 
		{
			...
			$arrResults = EventLead::getListForExport();
		}
		...
		if (count($arrResults) > 0) 
		{
			foreach ($arrResults as $key => $value) 
			{
				...
				$interested = 'N';
				if($value->chrInterest == 'M'){
						$interested = 'May be';
				}else if($value->chrInterest == 'Y'){
						$interested = 'Yes';
				}else{
						$interested = 'N';
				}

				$eventName = (isset($value->event->varTitle)) ? $value->event->varTitle:'-'; 
			...
			$this->createEventLeadExcel($data);
		}
		
	}

	/**
	 * This method create event lead excel sheet
	 * @return  xls file
	 * @since   2016-10-18
	 * @author  NetQuick
	 */	
	public function createEventLeadExcel($data)
	{

	Excel::create(Config::get('Constant.SITE_NAME').'-'. trans("template.eventleadModule.eventLeads") .'-'.date("dmy-h:i"), 
		...
		{			
				$excel->sheet(date('M-d-Y'), function($sheet) use($data) 
				{
					...
					$sheet->row(1, array(						
						...
						trans('template.eventleadModule.phone'),
						trans('template.common.interested'),
						trans('template.common.noofpeaople'),
						trans('template.eventleadModule.eventName'),
						trans('template.eventleadModule.comments'),
						trans('template.eventleadModule.ipAddress'),
						trans('template.eventleadModule.receivedDateTime')
					));

					$sheet->prependRow(array(
						Config::get('Constant.SITE_NAME').' '.trans("template.eventleadModule.eventLeads")
					));
