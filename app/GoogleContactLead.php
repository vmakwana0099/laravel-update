...
class GoogleContactLead extends Model {
     ...
     * @var string
     */
    protected $table = 'google_contact_lead';
    protected $fillable = [
        'id',
        'name',
        'varEmail',
    ...
    public static function getRecordById($id, $moduleFields = false) {
        $response = false;
        $moduleFields = ['id','name','varEmail','varDomain','varCompanyName','varPhoneNo','NoOfLicenses','txtUserMessage','chrPublish','chrDelete','varRefURL','varIpAddress','created_at','updated_at'];
    ...
    public static function getListForExport($selectedIds = false) {
        $response = false;
        $moduleFields = [ 'name', 'varEmail', 'varPhoneNo', 'txtUserMessage', 'varIpAddress', 'created_at'];
    ...
    function scopeFilter($query, $filterArr = false, $retunTotalRecords = false) {
        ...
        if (isset($filterArr['searchFilter']) && !empty($filterArr['searchFilter'])) {
            ...
            $data = $query->where('name', 'like', '%' . $filterArr['searchFilter'] . '%')->orwhere('varEmail', 'like', '%' . $emails . '%');
        }
