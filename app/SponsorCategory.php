...
class SponsorCategory extends Model
{
		...
		{
				$response = false;
				$response = Cache::tags(['SponsorCategory'])->get('getSponsorCatRecordIdByAliasID'.$aliasID);
				if(empty($response)){
					$response = Self::Select('id')->deleted()->publish()->checkAliasId($aliasID)->first();
					Cache::tags(['SponsorCategory'])->forever('getSponsorCatRecordIdByAliasID'.$aliasID, $response);
				}
				return $response;
		}

		/**
		 * This method handels retrival of front latest sponsor list
		 * @return  Object
		 * @since   2017-10-13
		 * @author  NetQuick
		 */
		...
		{
				$response = false;
				$sponsorFields = ['id', 'varTitle'];
				$response = Cache::tags(['SponsorCategory'])->get('getFrontSponsorCatList');
				if(empty($response)){
					$response = Self::getFrontRecords($sponsorFields)
					->deleted()
					->publish()
					->get()
					->pluck('varTitle', 'id');
					Cache::tags(['SponsorCategory'])->forever('getFrontSponsorCatList', $response);
				}
				return $response;
		}
		...
		/**
		 * This method handels sponsor-category relation
		 * @return  Object
		 * @since   2016-07-14
		 * @author  NetQuick
		 */
		...
		{
				$response = false;
				$response = $this->hasOne('App\Sponsor', 'id', 'intCategoryId');
				return $response;
		}

		/**
		 * This method handels sponsor sub-category relation
		 * @return  Object
		 * @since   2016-07-14
		 * @author  NetQuick
		 */
		...
		{
				$response = false;
				$response = $this->hasOne('App\SponsorCategory', 'id', 'intParentCategoryId');
				return $response;
		}
		...
		{
				$response = false;
				$response = $this->belongsTo('App\SponsorCategory', 'intParentCategoryId', 'id');
				return $response;
		}
