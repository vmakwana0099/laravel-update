...
class EventCategory extends Model
{
		...
		{
				$response = false;
				$response = Cache::tags(['EventCategory'])->get('getEventCatRecordIdByAliasID'.$aliasID);
				if(empty($response)){
						$response = Self::Select('id')->deleted()->publish()->checkAliasId($aliasID)->first();
						Cache::tags(['EventCategory'])->forever('getEventCatRecordIdByAliasID'.$aliasID, $response);
				}
				return $response;
		}
		...
		public static function getFrontList()
		{
				...
				$response = Cache::tags(['EventCategory'])->get('getFrontEventCatList');
				if(empty($response)){
						$response = Self::getFrontRecords($serviceFields)
								...
								->get()
								->pluck('varTitle', 'id');
						Cache::tags(['EventCategory'])->forever('getFrontEventCatList', $response);
				}
				return $response;
		}
		...
		{
				$response = false;
				$response = $this->hasOne('App\Event', 'id', 'intCategoryId');
				return $response;
		}
		...
		{
				$response = false;
				$response = $this->hasOne('App\EventCategory', 'id', 'intParentCategoryId');
				return $response;
		}
		...
		{
				$response = false;
				$response = $this->belongsTo('App\EventCategory', 'intParentCategoryId', 'id');
				return $response;
		}
		...
		public static function getParentCategoryNameBycatId($id)
		{
				...
				$response       = Self::getPowerPanelRecords($categoryFields)->deleted()->whereIn('id', $id)->get();
				return $response;
		}
		/**
