...
class CareersCategory extends Model
{
		protected $table    = 'careers_category';
		protected $fillable = [
				'id',
				'intAliasId',
				'varTitle',
		...
		{
				$response = false;
				$response = Cache::tags(['CareerCategory'])->get('getCareerCatRecordIdByAliasID'.$aliasID);
				if(empty($response)){
					$response = Self::Select('id')->deleted()->publish()->checkAliasId($aliasID)->first();
					Cache::tags(['CareerCategory'])->forever('getCareerCatRecordIdByAliasID'.$aliasID, $response);
				}
				return $response;
		}
		...
		{
				$response = false;
				$careerFields = ['id', 'varTitle'];
				$response = Cache::tags(['CareerCategory'])->get('getFrontCareerCatList');
				if(empty($response)){
					$response = Self::getFrontRecords($careerFields)
					->deleted()
					->publish()
					->get()
					->pluck('varTitle', 'id');
					Cache::tags(['CareerCategory'])->forever('getFrontCareerCatList', $response);
				}
				return $response;
		}
		...
		/**
		 * This method handels career-category relation
		 * @return  Object
		 * @since   2016-07-14
		 * @author  NetQuick
		 */
		...
		{
				$response = false;
				$response = $this->hasOne('App\Careers', 'id', 'intCategoryId');
				return $response;
		}

		/**
		 * This method handels career sub-category relation
		 * @return  Object
		 * @since   2016-07-14
		 * @author  NetQuick
		 */
		...
		{
				$response = false;
				$response = $this->hasOne('App\CareerCategory', 'id', 'intParentCategoryId');
				return $response;
		}
		...
		{
				$response = false;
				$response = $this->belongsTo('App\CareersCategory', 'intParentCategoryId', 'id');
				return $response;
		}
		...
		public static function getParentCategoryNameBycatId($ids)
		{
				...
				$response       = Self::getPowerPanelRecords($categoryFields)->deleted()
									// ->get();
									// ->whereIn('id', $ids)->get();
									->where('id', $ids)->get();
				return $response;
