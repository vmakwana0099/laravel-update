...
class Casestudies extends Model {
   ...
   /* public static function getRecordIdByAliasID($aliasID) {
    ...
    public static function getLatestList($id = false) {
        $response = false;
        $moduleFields = ['id', 'varTitle', 'txtDescription', 'fkIntImgId', 'intAliasId'];
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['Casestudies'])->get('getFrontLatestCasestudyList_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->take(5)
                    ->get();
            Cache::tags(['Casestudies'])->forever('getFrontLatestCasestudyList_' . $id, $response);
        }
        return $response;
    }
