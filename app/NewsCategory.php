...
class NewsCategory extends Model
{
    ...
    {
        $response = false;
        $response = Cache::tags(['NewsCategory'])->get('getNewsCatRecordIdByAliasID'.$aliasID);
        if(empty($response)){
            $response = Self::Select('id')->deleted()->publish()->checkAliasId($aliasID)->first();
            Cache::tags(['NewsCategory'])->forever('getNewsCatRecordIdByAliasID'.$aliasID, $response);
        }
        return $response;
    }
    ...
    public static function getFrontList()
    {
        ...
        $response = Cache::tags(['NewsCategory'])->get('getFrontNewsCatList');
        if(empty($response)){
            $response = Self::getFrontRecords($serviceFields)
                ...
                ->get()
                ->pluck('varTitle', 'id');
            Cache::tags(['NewsCategory'])->forever('getFrontNewsCatList', $response);
        }
        return $response;
    }
    ...
    {
        $response = false;
        $response = $this->hasOne('App\News', 'id', 'intCategoryId');
        return $response;
    }
