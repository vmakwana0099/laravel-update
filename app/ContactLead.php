...
class ContactLead extends Model {
    ...
    public static function getContactType() {
        $response = DB::table('contact_type')
                ...
                ->get();
        $returnHtml = '';
        $returnHtml .="<select id=\"var_Category\" name=\"var_Category\" class=\"form-control\">";
    ...
    function scopeFilter($query, $filterArr = false, $retunTotalRecords = false) {
        ...
        if (isset($filterArr['searchFilter']) && !empty($filterArr['searchFilter'])) {
            ...
            $phoneno = MyLibrary::encrypt_decrypt('encrypt', $filterArr['searchFilter']);
            $data = $query->where('varName', 'like', '%' . $filterArr['searchFilter'] . '%')->orwhere('varEmail', 'like', '%' . $emails . '%')->orwhere('txtUserMessage', 'like', '%' . $filterArr['searchFilter'] . '%')->orwhere('varPhoneNo', 'like', '%' . $phoneno . '%');
        }
