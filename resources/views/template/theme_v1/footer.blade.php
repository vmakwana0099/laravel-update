                            ...
                            <div class="col-lg-5">
                                <div class="hits-prom-popup-cnt">
                                    <div class="hits-prom-popup-head">
                                        Over 15000+ Clients Trust Host IT Smart!
                                    </div>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-6 col-sm-7">
                    <div class="encrg-offers-cnt">
                        ...
                        <form id="ic_signupform" method="POST" action="https://app.icontact.com/icp/core/mycontacts/signup/designer/form/?id=243&cid=487250&lid=59596" captcha-key="6LeCZCcUAAAAALhxcQ5fN80W6Wa2K3GqRQK6WRjA" captcha-theme="light" new-captcha="true">
...
<a href="#home" class="scrollToTop " title="Scroll To Top">
...
{{-- <a href="#" class="support-chat" target="_blank" title="Chat With Us">
    ...
    function copyPromocCode(val) { var copyText = document.getElementById(val);
        copyText.select();
    ...
    setTimeout(function() { $("#lucky-draw-popup").modal('show') }, 1000);
    <?php } ?>
    $('#lucky-draw-popup').on('show.bs.modal', function(e) { $('#spinner-div').show();
        $('#lucky-draw-content').hide();
        $(".dedicatedoffercode").html("Copy");
        $('#lucky-draw-popup .modal-title').text('Spinning please wait...');
        setTimeout(function() { var offercode = $.ajax({ type: "GET", url: "getoffercode.php", async: false }).responseText;
            $("#dedicatedoffercode").val(offercode);
            $('#spinner-div').hide();
            $('#lucky-draw-content').show();
    ...
    function startOtpCountdown() {
        ...
        var timer = setInterval(function() {
                    ...
                    if (--duration < 0) {
                        clearInterval(timer);
                        display.text("00:00"); { {--display.prop("disabled", false);-- } }
    ...
    $(document).ready(function() { if ($('#signin-form-link').hasClass('active')) { $("#one").show();
            $("#two").hide() } else { $("#one").hide();
            $("#two").show() } if ($.cookie("header_deals_close") != "Y") { $.cookie("header_deals_close", "N") } if ($.cookie("header_deals_close") == "N") { $(".top-header").show() } });

    function hidetopdeals() { $(".top-header").hide();
        $.cookie("header_deals_close", "Y") }
    ...
    var loadDeferredStyles = function() { var addStylesNode = document.getElementById("deferred-styles"); var replacement = document.createElement("div");
    ...
    function openZopim() {
        window.$zopim || (function(d, s) {
            ...
            $.src = '//v2.zopim.com/?1lXmJusakiXODEAGNIP6Refz9a4trf7V';
            z.t = +new Date;
            $.
    ...
    $(window).scroll(function() {
        if ($(window).scrollTop() < 200) { $("#top_bar").css({ "position": "relative", "top": "0", "width": "100%", "z-index": "9999" }); } else { $("#top_bar").css({ "position": "fixed", "top": "0", "width": "100%", "z-index": "9999" }); }
    });
    ...
    function scrollFunction() { let o = document.getElementById("scrollToTopBtn");
        o && (document.body.scrollTop > 20 || document.documentElement.scrollTop > 20 ? o.style.display = "block" : o.style.display = "none") } window.onscroll = function() { scrollFunction() }, document.addEventListener("DOMContentLoaded", function() { let o = document.getElementById("scrollToTopBtn");
        o && o.addEventListener("click", function() { document.body.scrollTop = 0, document.documentElement.scrollTop = 0 }) });
    ...
    document.addEventListener("DOMContentLoaded", function() { let e = document.querySelectorAll(".accordion-toggle");
...
(function () {
  /*-----------------------------------------------------------
   *  1️⃣  WHICH PAGES SHOULD SHOW THE POPUP?
   *-----------------------------------------------------------*/
  ...
  const ALLOWED_PATHS = [
    ...
    '/servers/vps-hosting',
    '/servers/managed-vps-hosting',
    '/servers/windows-vps-hosting',
    '/servers/forex-vps-hosting',
    '/servers/dedicated-servers',
    '/hosting/website-builder',
    '/email/google-workspace-india',
    '/email/microsoft-office-365-suite',
    '/ssl-certificates',
    '/servers/vps-hosting-india',
    '/servers/linux-vps-hosting',
    '/web-hosting-ahmedabad',
  ];

  // OR: regex patterns – good for sections such as every page under /blog/
  const ALLOWED_PATTERNS = [
    /^\/blog\//,                 // any URL that starts with /blog/
    /^\/pricing\/[a-z-]+$/i,     // e.g. /pricing/basic, /pricing/pro
  ];

  const path = window.location.pathname;

  // helper → true if current URL is in the allow‑list
  function pageIsAllowed () {
    if (ALLOWED_PATHS.includes(path)) return true;
    return ALLOWED_PATTERNS.some(re => re.test(path));
  }

  if (!pageIsAllowed()) return;   // ⬅️  STOP here — no popup on this page!
  /*-----------------------------------------------------------
   *  2️⃣  EVERYTHING BELOW IS THE SAME LOGIC AS BEFORE
   *-----------------------------------------------------------*/

  const EXIT_ZONE_HEIGHT = 50;       // px
  const POPUP_DELAY_MS   = 20_000;   // 20 s
  const DISMISS_KEY      = 'hitsPopupDismissed';
  const TIMER_START_KEY  = 'hitsPopupTimerStart';

  const now         = Date.now();
  let timerStart    = sessionStorage.getItem(TIMER_START_KEY);

  if (!timerStart) { sessionStorage.setItem(TIMER_START_KEY, now); timerStart = now; }
  else             { timerStart = Number(timerStart); }

  const msElapsed    = now - timerStart;
  const msRemaining  = Math.max(0, POPUP_DELAY_MS - msElapsed);
  let popupReady     = msRemaining === 0;
  let popupShown     = false;

  if (!popupReady) setTimeout(() => { popupReady = true; }, msRemaining);
  ...
  document.addEventListener('mousemove', function (e) {
    if (
      ...
      e.clientY <= EXIT_ZONE_HEIGHT
    ) {
      $('#hits-prom-modal').modal('show');
      popupShown = true;
    }
  });

  $(document).on('click', '.hits-prom-close-btn', function () {
