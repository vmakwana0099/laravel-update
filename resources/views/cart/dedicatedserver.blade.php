                ...
                <div class="cart-1">
                    <div class="cart-heading">
                        @if(isset($cartItem['groupname']))<h1 class="cart-head">{{$cartItem['groupname']}}</h1>
                          &nbsp;<img alt="Configure" title="Configure" src="assets/images/config.svg" onclick="window.location.href='{{url('cart/config')}}?id={{$key}}';" style="cursor:pointer;">
                        @endif
                  ...
                  <?php 
                  $configPrice = 0;
                   if (!empty($cartItem['configfields'])) 
                       {
                        foreach ($cartItem['configfields'] as $field) {
                          if (isset($field['selectedOption'])) {
                            if (!empty($field['options'])) {
                              foreach ($field['options'] as $opt) {
                                if ($opt['id'] == $field['selectedOption']) {
                                  $configPrice += $opt['pricing']['price'][$cartItem['regperiod']];
                                }
                              }
                            }
                          }
                    ...
                    <div class="subtotal-main">
                        <div class="cart-subtotal text-right">
                            ...
                            <div class="cart-total-text">
                            Configuration:
                                <span class="green"><span class="rupes">{!! Config::get('Constant.sys_currency_symbol') !!}</span>{{$configPrice}}</span>
                            </div>
                            @endif
                            <div class="blue">
