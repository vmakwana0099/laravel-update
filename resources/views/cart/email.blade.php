...
<div class="cart-check-left" data-aos="fade-right" data-aos-easing="ease-out-back">
                <div class="cart-1">
                    <div class="cart-heading">
                        @if(isset($cartItem['groupname']))<h1 class="cart-head">{{$cartItem['groupname']}}</h1>
                        &nbsp;<img src="assets/images/config.svg" onclick="window.location.href='{{url('cart/config')}}?id={{$key}}';" style="cursor:pointer;">
                        @endif
                        <a onclick="removeCartItem('{{$key}}');" class="delete-icon" title="Delete"><i class="delete-i sprite-image"></i></a>
                        @if(empty($cartItem['domain']))
                        <div class="provide-text error domainerror">
                              Please provide domain name.
                              <a href="javascript:void(0)" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
                                <i class="notify sprite-image" title="Please Click on "Config" icon above."></i>
                                <ul class="dropdown-menu tooltip-link-show" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 21px, 0px); top: 0px; left: 0px; will-change: transform;">
                                  <li>Please Click on "Config" icon above.</li>
                                </ul>
                              </a>
                            </div>
                        @endif
            ...
            <span class="premium-text">{{$cartItem['planname']}}
                        <?php /*<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" class="tooltip-link"><i class="notify sprite-image" title="{{$cartItem['planname']}}"></i>
                  ...
                  <div class="m-top">
                      <div class="row">
                        @if(isset($cartItem['domain']))
                         ...
                         @if(count($cartItem['pricing']))
                            ...
                            @if($price->price > 0)
                             @php 
                                $yousave = $cartItem['pricing'][$cartItem['regperiod']]->wrongprice - ($cartItem['pricing'][$cartItem['regperiod']]->price / $cartItem['pricing'][$cartItem['regperiod']]->duration);
                         ...
                         <div class="col-xl-2 col-md-4 col-sm-6 col-4" id="sel_hosting_{{$key}}" > 
                            <div class="for-flex flex-wrap  @if($price->durationame == $cartItem['billingcycle']) active @endif">
                              <label style="display:none;"><input onclick="updateHostingItem('{{$key}}','{{$key2}}');" type="radio"  id="sel_hostingregister_{{$key}}_{{$key2}}" name="sel_hostingregister_{{$key}}" value="{{$price->durationame}}" @if($price->durationame == $cartItem['billingcycle']) checked @endif />{{$price->durationame}}</label>
                               ...
                               <div class="cart-box" onclick="$('#sel_hostingregister_{{$key}}_{{$key2}}').prop('checked',true);updateHostingItem('{{$key}}','{{$key2}}');">
                    ...
                    <div class="subtotal-main">
                        <div class="cart-subtotal text-right">
                            ...
                            <div class="blue">
                            You save <span class="rupes">{!! Config::get('Constant.sys_currency_symbol') !!}</span> {{$yousave * $cartItem['pricing'][$cartItem['regperiod']]->duration}}
                            </div>
                        </div>
                    </div>
                </div>
