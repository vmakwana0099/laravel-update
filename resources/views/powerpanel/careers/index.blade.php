...
<div class="col-md-12">
	 ...
	 <div class="portlet light portlet-fit portlet-datatable bordered">
		@if($iTotalRecords > 0)
			<div class="portlet-title select_box career_select_box">
				 <span class="title">{{ trans('template.common.filterby') }}:</span>
						...
						@permission('careers-category-list')											
								<select class="form-control bs-select select2" id="category_id" name="category_id">
									 ...
									 @if(count($CareersCategory)>0)
									 <option value=" ">--{{ trans('template.common.selectcategory') }}--</option>
										 @foreach($CareersCategory as $category)					 
					 ...
					 <i class="fa fa-refresh" aria-hidden="true"></i>
						...
						@permission('careers-create')   
							<div class="pull-right">
								 <a class="btn btn-green-drake" href="{{ url('powerpanel/careers/add') }}">{{ trans('template.careerModule.addCareer') }}</a>
							</div>
						@endpermission
				...
				<a class="btn btn-green-drake pull-right" id="careersRange" href="javascript:;" style="margin:0 15px 0 0"><i class="fa fa-search"></i></a>
				<div class="event_datepicker pull-right">
				 ...
				 <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.careerModule.listingNote') }}</p>
							 ...
							 <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
									<thead>
										<tr role="row" class="heading">											
											...
											<th width="12%" align="center">{{ trans('template.common.dateandtime') }}</th>
			...
			<!-- End: life time stats -->
		@else
			@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/careers/add')])
		@endif		
	 </div>
</div>
<!-- /.modal -->
...
window.site_url =  '{!! url("/") !!}';
var DELETE_URL =  '{!! url("/powerpanel/careers/DeleteRecord") !!}';
...
<script src="{{ url('resources/pages/scripts/careers-datatables-ajax.js') }}" type="text/javascript"></script>
