		...
		@if($iTotalRecords > 0)
			<div class="portlet-title select_box new_select_box">
				...
				<span class="btn btn-icon-only green-new" type="button" id="refresh" title="Reset">
					...
					@permission('show-category-create')
				 <div class="pull-right">
						<a class="btn btn-green-drake" href="{{ url('powerpanel/show-category/add') }}">
						{{ trans('template.showCategoryModule.addShowCategory') }}
						</a>
				 </div>
				 @endpermission
				 <div class="clearfix"></div>
				 <div class="portlet-body">
				 <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.showCategoryModule.listingNote') }}</p>
						<div class="table-container">
							 ...
							 @permission('show-category-delete')
			...
			<!-- End: life time stats -->
		@else
			@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/show-category/add')])
		@endif
	 </div>
</div>
<!-- /.modal -->
...
<script type="text/javascript">
	 window.site_url =  '{!! url("/") !!}';
	 var DELETE_URL =  '{!! url("/powerpanel/show-category/DeleteRecord") !!}';
</script>
...
<script src="{{ url('resources/pages/scripts/show-category-datatables-ajax.js') }}" type="text/javascript"></script>
