		...
		<div class="portlet light bordered">
			<div class="portlet-body">
							...
							<div class="col-md-12">
								<div class="tab-pane active" id="general">
									<div class="portlet-body form_pattern">
										<form method="POST" id="frmPhotoGallery" action="{{url('powerpanel/photo-gallery/add')}}">
											<input type="hidden" name="_token" value="{{ csrf_token() }}">
											<div class="form-body">
												<div class="row">
													<div class="col-md-12">
														<div class="form-group @if($errors->first('title')) has-error @endif form-md-line-input">
															{!! Form::text('title', null, array('maxlength' => 150, 'class' => 'form-control seoField maxlength-handler','data-url' => 'powerpanel/photo-album','placeholder' => trans('template.common.title'),'autocomplete'=>'off')) !!}
												...
												@permission('photo-album-list')
												<div class="row">
													<div class="col-md-12">
														<div class="form-group @if($errors->first('album_id')) has-error @endif">
															<label class="form_title" for="album_id">{{ trans('template.photoAlbum.selectAlbum') }}</label>
															<br>
															<select class="form-control bs-select select2" name="album_id">
																@if(count($photoAlbumObj)>0)
																<option value=NULL>--- {{ trans('template.photoAlbum.selectAlbum') }} ---</option>
																@foreach($photoAlbumObj as $category)															
																	<option data-alias="{{ $category['id'] }}" {{ (app('request')->input('album')==$category['id'] )?'selected':'' }} value="{{ $category['id'] }}" >{{ $category['varTitle'] }}</option>
																@endforeach
																@else
																<option value=" ">Please add Photo Album</option>
																@endif
															</select>
															<span class="help-block">
														...
														<div class="image_thumb">
															<div class="form-group {{ $errors->has('img_id') ? ' has-error' : '' }} ">
																...
																<div class="fileinput fileinput-new" data-provides="fileinput">
																	<div class="fileinput-preview thumbnail photo_gallery_image_img" data-trigger="fileinput" style="border:1px solid rgb(221, 221, 221)">
																	...
																	<div class="input-group">
																		<a class="media_manager" onclick="MediaManager.open('photo_gallery_image');"><span class="fileinput-new"></span></a>
																	</div>
																	<input class="form-control" type="hidden" id="photo_gallery_image" name="img_id" value="{{ Input::old('img_id') }}" />
											...
											<div class="form-actions">
												<div class="row">
													<div class="col-md-12">
														<button type="submit" name="saveandedit" class="btn btn-green-drake" value="saveandexit">{{ trans('template.common.save') }}</button>
				...
				<div class="select_box service_select_box">
					<label class="form_title">{{ trans('template.photoAlbum.albumFilter') }}: </label>&nbsp;
					<select class="form-control bs-select select2 bs-select" name="albumfilter">
						<option value=" ">{{ trans('template.photoAlbum.filterByAlbum') }}</option>
				...
				@endpermission
				<div class="posts">
					@include('powerpanel.photo_album.photo_gallery',(isset($photoAlbumObj)?$photoAlbumObj:''))
				</div>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
		...
		{		
			if($(this).find('option:selected').data('alias') == undefined){
				window.location="{{url('powerpanel/photo-gallery')}}";	
			}else{
				window.location="{{url('powerpanel/photo-gallery?album=')}}"+$(this).find('option:selected').data('alias');	
			}
...
<script src="{{ url('resources/pages/scripts/photo_gallery_validations.js') }}" type="text/javascript"></script>
...
<script src="{{ url('resources/pages/scripts/photo_gallery.js') }}" type="text/javascript"></script>
