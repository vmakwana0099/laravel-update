...
@if(Session::has('message'))
   ...
   <div class="portlet light portlet-fit portlet-datatable">
    @if($iTotalRecords > 0)
      <div class="portlet-title select_box service_select_box">
         ...
         <div class="pull-right">
            <a class="btn btn-green-drake" href="{{ url('powerpanel/photo-album/add') }}">{{ trans('template.photoAlbum.addPhotoAlbum') }}</a>
         </div>
         @endpermission 
         <div class="clearfix"></div>
         <div class="portlet-body">
         <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.photoAlbum.listingNote') }}</p>
               ...
               <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                  <thead>
                     <tr role="row" class="heading">                        
                        ...
                        <th width="20%" align="center">{{ trans('template.photoAlbum.gallery') }}</th>
    ...
    @else
      @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/photo-album/add')])
    @endif    
   </div>
</div>
...
<script type="text/javascript">
   window.site_url =  '{!! url("/") !!}';
   var DELETE_URL =  '{!! url("/powerpanel/photo-album/DeleteRecord") !!}';
</script>
...
<script src="{{ url('resources/pages/scripts/photo-album-datatables-ajax.js') }}" type="text/javascript"></script>
