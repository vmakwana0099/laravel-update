...
<div class="row">
	<div class="col-md-12">
		...
		@if($iTotalRecords > 0)
			<div class="portlet-title select_box new_select_box">
				...
				<div class="pull-right">
					<a class="btn btn-green-drake" href="{{ url('powerpanel/deals/add') }}">{{ trans('template.dealsModule.addDeals') }}</a>
				</div>
				@endpermission
				<a class="btn btn-green-drake pull-right" id="refresh" title="Click to reset filters" href="javascript:;" style="margin:0 15px 0 0"><i class="fa fa-refresh"></i></a>
				<a class="btn btn-green-drake pull-right" id="newsRange" href="javascript:;" style="margin:0 15px 0 0"><i class="fa fa-search"></i></a>
				<div class="event_datepicker pull-right">
					<div class="new_date_picker input-group input-large date-picker input-daterange" data-date-format="yyyy/mm/dd">
						<span class="input-group-addon"><i class="icon-calendar"></i></span>
						<input class="form-control datepicker" id="start_date" placeholder="{{ trans('template.common.from') }}" value="{{ Carbon\Carbon::today()->format(Config::get('Constant.DEFAULT_DATE_FORMAT')) }}" readonly="" type="text">
						<span class="input-group-addon to_addon"><i class="icon-calendar"></i></span>
						<input class="form-control datepicker" id="end_date" placeholder="To" value="{{ Carbon\Carbon::today()->format(Config::get('Constant.DEFAULT_DATE_FORMAT')) }}" readonly="" type="text">
					</div>
				</div>
			</div>

			<div class="portlet-body">
			<p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.dealsModule.listingNote') }}</p>
					...
					<table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
						<thead>
							<tr role="row" class="heading">
								...
								<th width="5%" align="left">{{ trans('template.common.description') }}</th>
								<!--<th width="5%" align="center">{{ trans('template.common.image') }}</th>-->								
								<th width="10%" align="left">{{ trans('template.common.startdate') }}</th>								
								<th width="10%" align="left">{{ trans('template.common.enddate') }}</th>								
								<!--<th width="5%" align="left">{{ trans('template.common.category') }} </th>-->
		...
		@else
			@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/deals/add')])
		@endif
	</div>
</div>
...
<script type="text/javascript">
	window.site_url =  '{!! url("/") !!}';
	var DELETE_URL =  '{!! url("/powerpanel/deals/DeleteRecord") !!}';
	...
	<script src="{{ url('resources/pages/scripts/deals-datatables-ajax.js') }}" type="text/javascript"></script>
