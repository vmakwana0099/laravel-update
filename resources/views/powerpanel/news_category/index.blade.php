...
<div class="col-md-12">
	 ...
	 <div class="portlet light portlet-fit portlet-datatable">
		@if($iTotalRecords > 0)
			<div class="portlet-title select_box new_select_box">
				...
				<select id="statusfilter" data-sort data-order class="form-control bs-select select2">
				 ...
				 <div class="pull-right">
						<a class="btn btn-green-drake" href="{{ url('powerpanel/news-category/add') }}">
						{{ trans('template.newsCategoryModule.addNewsCategory') }}
						</a>
				 </div>
				 @endpermission
				 <div class="clearfix"></div>
				 <div class="portlet-body">
				 <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.newsCategoryModule.listingNote') }}</p>
			...
			<!-- End: life time stats -->
		@else
			@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/news-category/add')])
		@endif
	 </div>
</div>
<!-- /.modal -->
...
<script type="text/javascript">
	 window.site_url =  '{!! url("/") !!}';
	 var DELETE_URL =  '{!! url("/powerpanel/news-category/DeleteRecord") !!}';
</script>
...
<script src="{{ url('resources/pages/scripts/news-category-datatables-ajax.js') }}" type="text/javascript"></script>
