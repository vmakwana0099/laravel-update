		...
		@endif
			...
			@if($iTotalRecords > 0)
				...
				<span class="btn btn-icon-only green-new" type="button" id="refresh" title="Reset">
					...
					@permission('static-block-create')
						<div class="pull-right">
							<a class="btn btn-green-drake" href="{{ url('powerpanel/static-block/add') }}">{{ trans('template.staticblockModule.add') }}</a>
						</div>				
					@endpermission
				</div>
				...
				<p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.staticblockModule.listingNote') }}</p>					
					<div class="table-container">
						...
						<table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="static_blocks_datatable_ajax">
							<thead>
								<tr role="row" class="heading">
									...
									<!-- <th width="30%" align="left">{{ trans('template.common.alias') }}</th> -->
									<th width="10%" align="center">{{ trans('template.common.description') }}</th>
									<th width="25%" align="center">{{ trans('template.common.dateandtime') }}</th>
									<th width="15%" align="center">{{ trans('template.common.publish') }}</th>
									<th width="8%" align="center">{{ trans('template.common.actions') }}</th>
								</tr>
							</thead>
							<tbody></tbody>
						</table>
						@permission('static-block-delete')
			...
			@else				
				@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/static-block/add')])
			@endif
...
@section('scripts')
	...
	<script src="{{ url('resources/pages/scripts/table-staticblocks-ajax.js') }}" type="text/javascript"></script>
