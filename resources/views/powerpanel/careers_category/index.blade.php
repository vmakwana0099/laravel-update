...
<div class="row">
	...
	@if(Session::has('message'))
	  ...
	  <div class="portlet light portlet-fit portlet-datatable">
		@if($iTotalRecords > 0)
			<div class="portlet-title select_box new_select_box">
			  <span class="title">{{ trans('template.common.filterby') }}:</span>
				...
				@permission('careers-category-create')
					<div class="pull-right">
						<a class="btn btn-green-drake" href="{{ url('powerpanel/careers-category/add') }}">{{ trans('template.careersCategoryModule.addCareersCategory') }}</a>
					</div>
				@endpermission
				<div class="clearfix"></div>
				<div class="portlet-body">
				<p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.careersCategoryModule.listingNote') }}</p>
					<div class="table-container">
					  <div class="table-actions-wrapper">
						...
						@permission('careers-category-delete')
		...
		@else
			@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/careers-category/add')])
		@endif
		</div>
	</div>
</div>@include('powerpanel.partials.deletePopup')
@endsection
@section('scripts')
<script type="text/javascript">
	 window.site_url =  '{!! url("/") !!}';
	 var DELETE_URL =  '{!! url("/powerpanel/careers-category/DeleteRecord") !!}';
</script>
...
<script src="{{ url('resources/pages/scripts/careers-category-datatables-ajax.js') }}" type="text/javascript"></script>
