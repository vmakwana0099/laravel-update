...
<div class="col-md-12">
	 ...
	 <div class="portlet light portlet-fit portlet-datatable">
		@if($iTotalRecords > 0)
				...
				<select id="statusfilter" data-sort data-order class="form-control bs-select select2">
				   ...
				   <option value="N">{{ trans('template.common.unpublish') }}</option>
					...
					<i class="fa fa-refresh" aria-hidden="true"></i>
						...
						@permission('sponsor-category-create')
				 <div class="pull-right">
						<a class="btn btn-green-drake" href="{{ url('powerpanel/sponsor-category/add') }}">
						{{ trans('template.sponsorCategoryModule.addSponsorCategory') }}
						</a>
				 </div>
				 @endpermission
				 <div class="clearfix"></div>
				 <div class="portlet-body">
				 <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.sponsorCategoryModule.listingNote') }}</p>
						<div class="table-container">
							 ...
							 <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
									<thead>
										 <tr role="row" class="heading">												
												...
												<th width="35%" align="left">{{ trans('template.common.name') }}</th>
							 ...
							 @permission('sponsor-category-delete')							 
			...
			<!-- End: life time stats -->
		@else
			@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/sponsor-category/add')])
		@endif		
	 </div>
</div>
<!-- /.modal -->
...
<script type="text/javascript">
	 window.site_url =  '{!! url("/") !!}';
	 var DELETE_URL =  '{!! url("/powerpanel/sponsor-category/DeleteRecord") !!}';
</script>
...
<script src="{{ url('resources/pages/scripts/sponsor-category-datatables-ajax.js') }}" type="text/javascript"></script>
