...
<div class="col-md-12">
	 ...
	 <div class="portlet light portlet-fit portlet-datatable bordered">
		@if($iTotalRecords > 0)
					 ...
					 <i class="fa fa-refresh" aria-hidden="true"></i>
						...
						@permission('services-create')   
							<div class="pull-right">
								 <a class="btn btn-green-drake" href="{{ url('powerpanel/services/add') }}">{{ trans('template.serviceModule.addService') }}</a>
							</div>
						@endpermission	
				 <div class="clearfix"></div>
				 <div class="portlet-body">
				 <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.serviceModule.listingNote') }}</p>
							 ...
							 <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
									<thead>
										<tr role="row" class="heading">											
											...
											<th width="8%" align="center">{{ trans('template.common.icon') }}</th>
											<th width="10%" align="center">{{ trans('template.common.order') }}</th>
											<th width="12%" align="center">{{ trans('template.common.featured') }}?</th>
			...
			<!-- End: life time stats -->
		@else
			@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/services/add')])
		@endif		
	 </div>
</div>
<!-- /.modal -->
...
<script src="{{ url('resources/pages/scripts/services-datatables-ajax.js') }}" type="text/javascript"></script>
