			...
			<div class="portlet-title select_box">
				...
				@permission('contact-info-create')
					<div class="pull-right">
						<a class="btn btn-green-drake" href="{{ url('powerpanel/contact-info/add') }}"> {{ trans('template.contactModule.addnewcontact') }}</a>
					</div>
				@endpermission
                               @endif 
			</div>
			<div class="portlet-body">
			<p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.contactModule.listingNote') }}</p>
				<div class="table-container">
					...
					<table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="contacts_datatable_ajax">
						<thead>
							<tr role="row" class="heading">								
								...
								<th width="15%" align="left">{{ trans('template.common.email') }}</th>
                                                                <th width="10%" align="center">{{ trans('template.common.details') }}</th>
					...
					@permission('contact-info-delete')						
							<!--<a href="javascript:;" class="btn-sm btn red btn-outline right_bottom_btn deleteMass">{{ trans('template.common.delete') }}</a>-->						
					@endpermission
				</div>
			</div>
			@else
