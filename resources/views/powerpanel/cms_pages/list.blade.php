...
<link href="{{ url('resources/global/plugins/tooltips/tooltip.css') }}" rel="stylesheet" type="text/css"/>
@endsection
@section('content')
		...
		<div class="portlet light portlet-fit portlet-datatable bordered">
			...
			<div class="portlet-title select_box ">
				...
				<div class="pull-right">
					<a class="btn btn-green-drake" href="{{ url('powerpanel/pages/add') }}">{{ trans('template.pageModule.add') }}</a>
				</div>
				@endpermission
			</div>
			<div class="portlet-body">
			<p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"><strong>{{ trans('template.common.note') }}:</strong> {{ trans('template.pageModule.notePage') }}</p>
			<p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.pageModule.listingNote') }}</p>
				<div class="table-container">
					...
					<table class="new_table_desing table table-striped table-bordered table-hover table-checkable dataTable" id="email_log_datatable_ajax">
						<thead>
							<tr role="row" class="heading">
								<th width="2%" align="center"><input type="checkbox" class="group-checkable"></th>
								<th width="30%" align="left">{{ trans('template.common.title') }}</th>
								<th width="15%" align="left">{{ trans('template.pageModule.module') }}</th>
								<th width="13%" align="center">{{ trans('template.powerPanelDashboard.webHits') }}</th>
								<th width="15%" align="center">{{ trans('template.powerPanelDashboard.mobileHits') }}</th>
...
<script src="{{ url('resources/global/plugins/tooltips/tooltip.js') }}" type="text/javascript"></script>
<script src="{{ url('resources/pages/scripts/table-cms-pages-ajax.js') }}" type="text/javascript"></script>
