...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
            ...
            <div class="portlet-title select_box service_select_box">
                ...
                <select id="statusfilter" data-sort data-order class="form-control bs-select select2">
                    ...
                    <option value="{{$status['id']}}">{{$status['text']}}</option>
                        @endforeach
                    @endif
                    @php /* <option value="Y">{{ trans('template.common.publish') }}</option>
                    <option value="N">{{ trans('template.common.unpublish') }}</option> */ @endphp
                </select>
                ...
                <select class="form-control bs-select select2" id="category_id" name="category_id">
                   ...
                   <!-- <option value="{{ $category->id }}" {{ ($category->id == Input::old('category_id') || app('request')->input('category')==$category->id)? 'selected' : '' }}>{{ $category->varTitle }}</option> -->
                    <option value="{{ $category->id }}" @if(old('category_id') == $category->id) selected @endif> {{ $category->varTitle }} </option>
              ...
              <i class="fa fa-refresh" aria-hidden="true"></i>
                ...
                <div class="pull-right">
                    <a class="btn btn-green-drake" href="{{ url('powerpanel/projects/add') }}">{{ trans('template.projectModule.addProject') }}</a>
                </div>
                @endpermission
                <div class="clearfix"></div>
                <div class="portlet-body">
                    <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.projectModule.listingNote') }}</p>
                        ...
                        <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                            <thead>
                                <tr role="row" class="heading">									
                                    ...
                                    <th width="12%" align="left">{{ trans('template.common.title') }}</th>
                                    <th width="16%" align="center">{{ trans('template.common.shortdescription') }}</th>
                                    <th width="5%" align="center">{{ trans('template.common.image') }}</th>
                                    <th width="10%" align="center">{{ trans('template.common.category') }}</th>
                                    <th width="10%" align="center">Status</th>
                                    <th width="12%" align="center">{{ trans('template.common.order') }}</th>
                                    <th width="18%" align="center">{{ trans('template.common.publish') }}</th>
                                    <th width="12%" align="right">{{ trans('template.common.actions') }}</th>
                                </tr>
                            </thead>
                            <tbody> </tbody>
                        </table>
            ...
            @else
            @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/projects/add')])
            @endif		
        </div>
    </div>
</div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/projects/DeleteRecord") !!}';
...
<script src="{{ url('resources/pages/scripts/projects-datatables-ajax.js') }}" type="text/javascript"></script>
