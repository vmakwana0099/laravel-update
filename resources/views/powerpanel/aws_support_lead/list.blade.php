                    ...
                    <table class="new_table_desing table table-striped table-bordered table-hover table-checkable dataTable" id="datatable_ajax">
                        <thead>
                            <tr role="row" class="heading">
                                ...
                                <th width="10%" align="left">{{ trans('template.awssupportleadModule.firstname') }}</th>							
                                <th width="10%" align="left">{{ trans('template.awssupportleadModule.lastname') }}</th>							
                                <th width="10%" align="left">{{ trans('template.common.email') }}</th>
                                <th width="10%" align="center">{{ trans('template.common.phoneno') }}</th>
                                <th width="15%" align="center">{{ trans('template.awssupportleadModule.state') }}</th>
                                <th width="15%" align="center">{{ trans('template.awssupportleadModule.company') }}</th>
                                <th width="10%" align="center">{{ trans('template.awssupportleadModule.comments') }}</th>
                                <th width="20%" align="center">{{ trans('template.common.received_date_time') }}</th>
                            </tr>
                        </thead>
                        <tbody></tbody>
                    </table>				
                    @permission('aws-support-leads-delete')
                    <a href="javascript:;" class="btn-sm btn btn-outline red right_bottom_btn deleteMass">{{ trans('template.common.delete') }}</a>
                    @endpermission
                    <a href="#selectedRecords" class="btn-sm btn btn-green-drake right_bottom_btn ExportRecord" data-toggle="modal">{{ trans('template.eventleadModule.export') }}</a>				
                </div>
            </div>
            @else
            @include('powerpanel.partials.addrecordsection',['marketlink' => '', 'type'=>'aws support'])
            @endif
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-vertical">	
            <div class="modal-content">
                ...
                <div class="modal-body text-center">{{ trans('template.eventleadModule.noExport') }} </div>
    ...
    <div class="modal-dialog">
        <div class="modal-vertical">	
            <div class="modal-content">
                ...
                <div class="modal-body text-center">{{ trans('template.eventleadModule.recordsExport') }}</div>
                <div class="modal-footer">
                    <div align="center">
                        <div class="md-radio-inline">
                            <div class="md-radio">
                                ...
                                <label for="selected_records">
                                    ...
                                    <span class="box"></span> {{ trans('template.eventleadModule.selectedRecords') }}
                                </label>
                            </div>
                            <div class="md-radio">
                                ...
                                <label for="all_records">
                                    <span class="inc"></span>
                                    <span class="check"></span>
                                    <span class="box"></span>{{ trans('template.eventleadModule.allRecords') }} 
                                </label>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-vertical">	
            <div class="modal-content">
                ...
                <div class="modal-body text-center">{{ trans('template.eventleadModule.leastRecord') }} </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-green-drake" data-dismiss="modal">{{ trans('template.common.ok') }} </button>
                </div>
            </div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/aws-support-leads/DeleteRecord") !!}';
</script>
...
<script src="{{ url('resources/pages/scripts/awssupportlead-datatables-ajax.js') }}" type="text/javascript"></script>
