    ...
    @if($iTotalRecords > 0)
      <div class="portlet-title select_box service_select_box">
        ...
        <i class="fa fa-refresh" aria-hidden="true"></i>
            ...
            @permission('video-album-create') 
         <div class="pull-right">
            <a class="btn btn-green-drake" href="{{ url('powerpanel/video-album/add') }}">{{ trans('template.videoAlbum.addVideoAlbum') }}</a>
         </div>
         @endpermission 
         <div class="clearfix"></div>
         <div class="portlet-body">
         <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.videoAlbum.listingNote') }}</p>
               ...
               <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                  <thead>
                     <tr role="row" class="heading">                        
                        ...
                        <th width="16%" align="center">{{ trans('template.common.description') }}</th>
                        <th width="5%" align="center">{{ trans('template.common.image') }}</th>
                        <th width="20%" align="center">{{ trans('template.common.gallery') }}</th>
                        <th width="15%" align="center">{{ trans('template.common.order') }}</th>
      ...
      <!-- End: life time stats -->
    @else
      @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/video-album/add')])
    @endif    
   </div>
</div>
...
<script type="text/javascript">
   window.site_url =  '{!! url("/") !!}';
   var DELETE_URL =  '{!! url("/powerpanel/video-album/DeleteRecord") !!}';
</script>
...
<script src="{{ url('resources/pages/scripts/video-album-datatables-ajax.js') }}" type="text/javascript"></script>
