...
<div class="row">
	...
	@if(Session::has('message'))
		...
		<div class="portlet light portlet-fit portlet-datatable bordered">
			...
			<div class="portlet-title select_box">
			  ...
			  <i class="fa fa-refresh" aria-hidden="true"></i>
				...
				<div class="pull-right">
					<a class="btn btn-green-drake" href="{{ url('powerpanel/team/add') }}"> {{ trans('template.teamModule.addTeamMember') }}</a>
				</div>
				@endpermission			
			</div>
			...
			<p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.teamModule.listingNote') }}</p>					
					...
					<table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
						<thead>
							<tr role="row" class="heading">								
								...
								<th width="12%">{{ trans('template.teamModule.designation') }}</th>
			...
			@else
				@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/team/add')])
			@endif
		</div>
</div>
</div>
...
@section('scripts')
	...
	<script src="{{ url('resources/pages/scripts/team-datatables-ajax.js') }}" type="text/javascript"></script>
