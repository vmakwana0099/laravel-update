						...
						<div class="table-container">
							 ...
							 <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
								<thead>
									...
									<th width="10%" align="left">{{ trans('template.restaurantReservations.name')  }}</th>
									<th width="10%" align="left">{{ trans('template.restaurantReservations.email')  }}</th>
									<th width="15%" align="center">{{ trans('template.restaurantReservations.phone') }}</th>
									<th width="12%" align="center">People</th>
									<!-- <th width="2%" align="center">{{ trans('template.restaurantReservations.splReq') }}</th>
									<th width="10%" align="center">{{ trans('template.restaurantReservations.occasions') }}</th> -->
									<th width="15%" align="center">{{ trans('template.common.date_time') }}</th>
									<th width="15%" align="center">{{ trans('template.common.ip') }} </th>
									</tr>
								</thead>
								<tbody> </tbody>
							 </table>

							@permission('newsletter-lead-delete')
								<a href="javascript:;" class="btn-sm btn btn-outline red right_bottom_btn deleteMass">{{ trans('template.common.delete') }} </a>
							@endpermission
							<a href="#selectedRecords" class="btn-sm btn btn-green-drake right_bottom_btn ExportRecord" data-toggle="modal"> {{ trans('template.restaurantReservations.export') }}</a>

					</div>
				 </div>
				@else
					@include('powerpanel.partials.addrecordsection',['marketlink' => 'https://www.netclues.com/social-media-marketing', 'type'=>'reservation'])
				@endif
			</div>
			<!-- End: life time stats -->
	 </div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">
			<div class="modal-content">
					...
					<div class="modal-body text-center">{{ trans('template.restaurantReservations.noExport') }}</div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">
			<div class="modal-content">
					...
					<div class="modal-body text-center">{{ trans('template.restaurantReservations.recordsExport') }}</div>
					<div class="modal-footer">
						<div align="center">
							<div class="md-radio-inline">
								<div class="md-radio">
									...
									<label for="selected_records">
										...
										<span class="box"></span> {{ trans('template.restaurantReservations.selectedRecords') }}
									</label>
								</div>
								<div class="md-radio">
									...
									<label for="all_records">
										<span class="inc"></span>
										<span class="check"></span>
										<span class="box"></span> {{ trans('template.restaurantReservations.allRecords') }}
									</label>
								</div>
							</div>
						</div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">
			<div class="modal-content">
				...
				<div class="modal-body text-center">{{ trans('template.restaurantReservations.leastRecord') }}</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-green-drake" data-dismiss="modal">{{ trans('template.common.ok') }}</button>
				</div>
			</div>
...
<script type="text/javascript">
	window.site_url =  '{!! url("/") !!}';
	var DELETE_URL =  '{!! url("/powerpanel/restaurant-reservations/DeleteRecord") !!}';
</script>
...
<script src="{{ url('resources/pages/scripts/restaurant-reservations-ajax.js') }}" type="text/javascript"></script>
