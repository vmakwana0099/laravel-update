...
<div class="row">
	...
	@if(Session::has('message'))
		...
		<div class="portlet light portlet-fit portlet-datatable bordered">
			...
			<div class="portlet-title select_box">
			  ...
			  <i class="fa fa-refresh" aria-hidden="true"></i>
				...
				<div class="pull-right">
					<a class="btn btn-green-drake" href="{{ url('powerpanel/client/add') }}"> {{ trans('template.clientModule.addClientMember') }}</a>
				</div>
				@endpermission			
			</div>
			
			<div class="portlet-body">
			<p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.clientModule.listingNote') }}</p>					
					...
					<table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
						<thead>
							<tr role="row" class="heading">								
								...
								<th width="30%">{{ trans('template.common.name') }}</th>
			...
			@else
				@include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/client/add')])
			@endif
		</div>
</div>
</div>
...
@section('scripts')
	<script type="text/javascript">
		window.site_url =  '{!! url("/") !!}';
		var DELETE_URL =  '{!! url("/powerpanel/client/DeleteRecord") !!}';
	...
	<script src="{{ url('resources/pages/scripts/client-datatables-ajax.js') }}" type="text/javascript"></script>
