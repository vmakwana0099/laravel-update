    ...
    <div class="col-md-12">
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
            ...
            <div class="portlet-title select_box">
                ...
                <select id="featuredfilter" data-sort data-order class="bs-select select2">
                    <option value=" ">{{ trans('template.common.selectstatus') }}</option>
                    <option value="Y">{{ trans('template.tldModule.featured') }}</option>
                </select>
                <span class="btn btn-icon-only green-new" type="button" id="refresh" title="Reset">
                    <i class="fa fa-refresh" aria-hidden="true"></i>
                </span>
                @permission('tld-create')	
                <div class="pull-right">
                    <a class="btn btn-green-drake" href="{{ url('powerpanel/tld/add') }}"> {{ trans('template.tldModule.addTldMember') }}</a>
                </div>
                @endpermission			
            </div>
            ...
            <div class="portlet-body">
                <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.tldModule.listingNote') }}</p>					
                    ...
                    <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                        <thead>
                            <tr role="row" class="heading">								
                                ...
                                <th width="15%">{{ trans('template.common.name') }}</th>
                                <th width="5%" align="left">{{ trans('template.common.description') }}</th>								
                                <th width="8%">{{ trans('template.tldModule.featured') }}</th>
                                <th width="10%">{{ trans('template.common.image') }}</th>
                                <th width="10%">{{ trans('template.powerPanelDashboard.webHits') }}</th>
                                <th width="10%">{{ trans('template.powerPanelDashboard.mobileHits') }}</th>
            ...
            @else
            @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/tld/add')])
            @endif
        </div>
    </div>
</div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/tld/DeleteRecord") !!}';
...
<script src="{{ url('resources/pages/scripts/tld-datatables-ajax.js') }}" type="text/javascript"></script>
