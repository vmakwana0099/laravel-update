...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
            {!! Form::hidden('categoryfilter','', array('id' => 'categoryfilter')) !!}
            @if($iTotalRecords > 0)
            <div class="portlet-title select_box service_select_box">
                ...
                @permission('casestudy-create')
                <div class="pull-right">
                    <a class="btn btn-green-drake" href="{{ url('powerpanel/casestudy/add') }}">{{ trans('template.casestudyModule.addCasestudy') }}</a>
                </div>
                @endpermission
                <div class="clearfix"></div>
                <div class="portlet-body">
                    <p style="background:rgba(16, 128, 242,0.08);color:rgba(16, 128, 242, 1);width:100%;padding:10px 15px"> {{ trans('template.casestudyModule.listingNote') }}</p>
                        ...
                        <table class="new_table_desing table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                            <thead>
                                <tr role="row" class="heading">
                                    ...
                                    <th width="12%" align="center">{{ trans('template.common.shortdescription') }}</th>
                                    <th width="5%" align="center">{{ trans('template.common.image') }}</th> 
                                    <th width="10%" align="center">{{ trans('template.powerPanelDashboard.webHits') }}</th>
                                    <th width="10%" align="center">{{ trans('template.powerPanelDashboard.mobileHits') }}</th>
            ...
            @else
            @include('powerpanel.partials.addrecordsection',['type'=>Config::get('Constant.MODULE.TITLE'), 'adUrl' => url('powerpanel/casestudy/add')])
            @endif      
        </div>
    </div>
</div>
...
<script type="text/javascript">
    window.site_url = '{!! url("/") !!}';
    var DELETE_URL = '{!! url("/powerpanel/casestudy/DeleteRecord") !!}';
...
<script src="{{ url('resources/pages/scripts/casestudy-datatables-ajax.js') }}" type="text/javascript"></script>
